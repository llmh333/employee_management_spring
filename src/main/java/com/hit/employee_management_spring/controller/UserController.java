...
public class UserController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "403", description = "Access denied — not admin and not own profile", content = @Content(schema = @Schema(implementation = RestData.class))),
            @ApiResponse(responseCode = "404", description = "User not found", content = @Content(schema = @Schema(implementation = RestData.class)))
    })
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Paginated user list", content = @Content(schema = @Schema(implementation = PaginationResponseDto.class))),
            @ApiResponse(responseCode = "403", description = "Access denied", content = @Content(schema = @Schema(implementation = RestData.class)))
    })
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Validation error", content = @Content(schema = @Schema(implementation = RestData.class))),
            @ApiResponse(responseCode = "409", description = "Username or email already exists", content = @Content(schema = @Schema(implementation = RestData.class)))
    })
    ...
    @Operation(summary = "Update user roles (admin)", description = "Updates roles for a specific user. ADMIN only.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "User roles updated", content = @Content(schema = @Schema(implementation = UserResponseDto.class))),
            @ApiResponse(responseCode = "403", description = "Access denied", content = @Content(schema = @Schema(implementation = RestData.class))),
            @ApiResponse(responseCode = "404", description = "User not found", content = @Content(schema = @Schema(implementation = RestData.class)))
    })
    @PatchMapping(UrlConstant.User.UPDATE_USER_ROLE)
    public ResponseEntity<?> updateUserRole(@RequestBody @Valid UpdateUserRoleRequestDto requestDto) {
        UserResponseDto responseDto = userService.updateUserRole(requestDto);
        return ApiResponseUtil.success(responseDto);
    }
    ...
    public ResponseEntity<?> deleteUserById(
        ...
        if (!resultDelete) {
            String message = messageSource.getMessage(ErrorMessage.Auth.UNAUTHENTICATED, null,
